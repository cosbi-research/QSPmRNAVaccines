function [t_record, y_record, pars]=simulation_tris(Liang_fit, par_fit, Liang_data, mRNA0_g, Ag_MW, schedule_test, t_end, Tolerance)
...
% 1st dose
[t_1,y_1]=ode15s(@(t,Y) model_equations(t,Y,pars), [0 schedule_test(1)], IC_0, options);

% 2nd dose
IC_1 = y_1(end,:);
IC_1(1) = IC_1(1)+pars.mRNA0_pmol;   % 2nd dose
[t_2,y_2]=ode15s(@(t,Y) model_equations(t,Y,pars), [0 schedule_test(2)-schedule_test(1)], IC_1, options);

% 3rd dose
IC_2 = y_2(end,:);
IC_2(1) = IC_2(1)+pars.mRNA0_pmol;   % 3rd dose
[t_3,y_3]=ode15s(@(t,Y) model_equations(t,Y,pars), [0 t_end-schedule_test(2)], IC_2, options);

% merging the simulations
t_record = [t_1(1:(end-1)); t_2(1:end-1)+t_1(end); t_3(1:end)+t_2(end)+t_1(end)]; % simulation time vector
y_record = [y_1(1:(end-1),:); y_2(1:end-1,:); y_3(1:end,:)]; % simulation dynamics vector
