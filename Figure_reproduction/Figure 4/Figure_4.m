...
admin_times = [21 291]; % days at which the second and the booster doses are administered
...
t_end1 = admin_times(1)+38*7; % simulation end-point (38 weeks after the second dose)
t_end2 = admin_times(2)+38*7; % simulation end-point (38 weeks after the booster dose)
Tolerance = 1e-10; % Tolerance for the integration (MATLAB default is 1e-6)


%% simulation of the dynamics

[t_record10, y_record10, pars10] = simulation(Liang_fit, opt_param, Liang_data, dose_g/3, Ag_MW, admin_times(1), t_end1, Tolerance);
[t_record20, y_record20, pars20] = simulation(Liang_fit, opt_param, Liang_data, dose_g*2/3, Ag_MW, admin_times(1), t_end1, Tolerance);
[t_record30, y_record30, pars30] = simulation_tris(Liang_fit, opt_param, Liang_data, dose_g, Ag_MW, admin_times, t_end2, Tolerance);
...
IgG10 = ((Ab_tot_BL10*10^(-12))*pars10.MW_Ab*10^9)/(pars10.V_BL*10^3); %ng/mL

Ab20 = y_record20(:,51+8*pars20.J:50+9*pars20.J);
Ab_tot_BL20 = sum(Ab20, 2);
IgG20 = ((Ab_tot_BL20*10^(-12))*pars20.MW_Ab*10^9)/(pars20.V_BL*10^3); %ng/mL
...
load NaaberData.mat % data are in AU/mL
...
load TakeuchiData.mat % data are in AU/mL
...
hold on
j_1 = errorbar(timepoints_Pfizer(2:end), fact_of_conv.*points10ng(2:end), fact_of_conv.*points10ng(2:end) - fact_of_conv.*low9510ng(2:end), ...
    fact_of_conv.*upp9510ng(2:end) - fact_of_conv.*points10ng(2:end), 'o', 'Color', color(2,:), 'LineWidth', 1.5, 'MarkerSize', 8);
j = semilogy(timepoints_Pfizer(2:end), fact_of_conv.*points10ng(2:end), 'o', 'Color', color(2,:), 'LineWidth', 1.5, 'MarkerSize', 8);
xlim([0 t_end1])
ylim([11 247000])
xlabel('days')
...
ax = gca; % current axes
ax.FontSize = 14;
legend([m j], {'model prediction', 'Sahin et al. (10μg)'}, 'FontSize', 14, Location='south')   


subplot(2,2,2);
m = semilogy(t_record20,IgG20,'Color', color(3,:),'LineWidth',2);
hold on
j_1 = errorbar(timepoints_Pfizer(2:end), fact_of_conv.*points20ng(2:end), fact_of_conv.*points20ng(2:end) - fact_of_conv.*low9520ng(2:end), fact_of_conv.*upp9520ng(2:end) - fact_of_conv.*points20ng(2:end), 'o', 'Color', color(3,:), 'LineWidth', 1.5, 'MarkerSize', 8);
j = semilogy(timepoints_Pfizer(2:end), fact_of_conv.*points20ng(2:end), 'o', 'Color', color(3,:), 'LineWidth', 1.5, 'MarkerSize', 8);
xlim([0 t_end1])
ylim([11 247000])
xlabel('days')
ylabel('RBD antibodies [ng/mL]')
ax = gca; % current axes
ax.FontSize = 14;
legend([m j], {'model prediction', 'Sahin et al. (20μg)'}, 'FontSize', 14, Location='south')   
